...
const EventDisplay: React.FC<EventDisplayProps> = ({ title, events, onCopy }) => {
  ...
  const parsedEvents: Event[] = events.map((event) => (typeof event === "string" ? JSON.parse(event) : event));
...
const eventCode = `window.dataLayer.push({
  ...
  style={{
    ...
    overflowY: "auto",
    border:
      activeBorders[codeKey] // Apply border color for "Copy Code"
    ...
    boxShadow:
      activeBorders[codeKey] // Apply glowing effect for "Copy Code"
